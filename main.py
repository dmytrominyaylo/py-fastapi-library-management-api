...
@app.get(path="/authors/", response_model=list[schemas.AuthorSchema])
def read_authors(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    return crud.get_all_authors(skip=skip, limit=limit, db=db)


@app.get(path="/authors/{author_id}/", response_model=schemas.AuthorSchema)
...
@app.post(path="/authors/", response_model=schemas.AuthorSchema)
def create_author(author_schema: schemas.AuthorCreateSchema, db: Session = Depends(get_db)):
    author = crud.get_author_by_name(name=author_schema.name, db=db)
    if author:
...
@app.get(path="/books/", response_model=list[schemas.BookSchema])
...
@app.get(path="/books/{book_id}/", response_model=schemas.BookSchema)
...
@app.post(path="/authors/{author_id}/books/", response_model=schemas.BookSchema)
